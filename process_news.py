...
def process_news():
    ...
    print(article.img)
    print(article.img_desc)
    print(article.money_mention)
    print(article.link)
    """
...
def create_output_file(article):
    header = ["Title", "Date", "Author", "Image filename", "Image description", "Money mention", "Link"]
    data = [article.title, article.date, article.author, article.img, article.img_desc, article.money_mention, article.link]
    ...
    # Append the header and data rows
